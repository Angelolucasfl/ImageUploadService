...
const fileSizeLimiter = (req: Request, res: Response, next: NextFunction) => {
  const files = req.files as Record<string, expressFileupload.UploadedFile | expressFileupload.UploadedFile[]> | null | undefined;
  ...
  if (files) {
    ...
    filesArray.forEach((fileOrArray) => {
      ...
      fileArray.forEach((file: expressFileupload.UploadedFile) => {
